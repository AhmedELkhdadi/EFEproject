<%@ page language="java" contentType="text/html; charset=ISO-8859-1" pageEncoding="ISO-8859-1" %>
	...
	<body>
							...
							<div class="card-body">
								<div class="row no-gutters align-items-center">
									<div style="float : left;" class="col-auto">
										<img id="logoImage"
											src="${pageContext.request.contextPath}/assets/img/EnterpriseLogos/${standRep.getEntreprise().getLogo()}?time=${System.currentTimeMillis()}"
											alt="Image" style="height:90px;width:90px; border-radius: 50%;">

									</div>
									<div class="col mr-2 justify-content-center">
										<div align="center" style="margin:auto">
											<h2 class="  text-primary mb-1">WELCOME TO <span class="text-uppercase">${
													standRep.getEntreprise().getName() }</span>'S STAND !</h2>
										</div>
				...
				<div class="row">
					<div class="col-xl-9 col-lg-9 col-sm-12">
						...
						<div class="card border-0 shadow mb-3">
							<img height="250px"
								src="${pageContext.request.contextPath}/assets/img/EnterpriseBanners/${standRep.getEntreprise().getBannerImage()}?time=${System.currentTimeMillis()}"
								class="card-img-top" id="bannerImage" alt="...">
							...
							<div class="card-body" style="height : 315px; overflow-y: scroll;">
								...
								<p><strong>Representative: </strong><span class="text-danger">${standRep.getName()}
										${standRep.getFname()}.</span></p>
								...
								<c:if test="${ eventState == 1 }">
									<c:if test="${ currentActivity.getName() == 'Stands and Workshops' }">
										<c:choose>
											<c:when
												test="${ user.getStatus() != 2 && standRep.getEntreprise().getMeetLink() != null  }">
												<span>Join Representative for a talk: </span>
												<strong class="text-info">
													<a href="${ standRep.getEntreprise().getMeetLink() }"
														target="_blank">${ standRep.getEntreprise().getMeetLink() }</a>
												</strong>
											</c:when>
												...
												<c:if test="${standRep.isApproved() != null}">
													<c:if test="${standRep.isApproved() == true}">
														<div class="w-100 d-flex align-items-center">
															<input class="form-control" id="meetLink"
																value="${ standRep.getEntreprise().getMeetLink()}"
																placeholder="Enter a meet Link">
															<button class="btn btn-primary ml-2" style="display:none"
																id="confLink">Validate</button>
														</div>
													</c:if>
												</c:if>
											</c:when>
						...
						<div class="card shadow border-0">
							<div class="card-body pb-0">
								...
								<div style="margin-left:-5px">
									<h6 class="border-bottom pb-2 mb-0">Other Enterprise Informations</h6>
								</div>
								<div class="d-flex text-muted pt-3">
									...
									<div class="shadow bg-danger text-white"
										style="height: 40px; width: 40px; border-radius: 50%;  display: flex; justify-content: center; align-items: center;">
										<i class="fas fa-briefcase"></i>
									</div>
									<p class="pb-3 mb-0 small lh-sm border-bottom ml-2">
										<strong class="d-block text-gray-dark ">Domain </strong>
										${standRep.getEntreprise().getDomain()}.
									</p>
								</div>
								<div class="d-flex text-muted pt-3">
									<div class="shadow text-white"
										style="height: 40px; width: 40px; border-radius: 50%; background-color: blue;  display: flex; justify-content: center; align-items: center;">
										<i class="fas fa-map-marker-alt"></i>
									</div>
									<p class="pb-3 mb-0 small lh-sm border-bottom ml-2">
										<strong class="d-block text-gray-dark">Location </strong>
										${standRep.getEntreprise().getLocation()}.
									</p>
								</div>
								<div class="d-flex text-muted pt-3 ">
									<div class="shadow bg-success text-white"
										style="height: 40px; width: 40px; border-radius: 50%;  display: flex; justify-content: center; align-items: center;">
									...
									<p class="pb-3 mb-0 small lh-sm border-bottom ml-2">
										...
										${standRep.getEntreprise().getEmail()}
									</p>
								</div>
											...
											<div class="card-body">
												...
												<c:choose>
													<c:when test="${ sentResume == null }">
														<p>Send a good written resume, this stand's representative will
															view it and might contact you for an opportunity</p>
														<div style="margin-top:-15px" id="resumeFormDiv">
															<small class="text-secondary">Only 'pdf' format is
																accepted</small>
															<form id="resumeForm" method="#" action="post">
																<input type="hidden" name="standIdResume"
																	value="${ standRep.getEntreprise().getId() }" />
																<input type="hidden" name="userNameResume"
																	value="${ user.getName() } ${ user.getFname() }" />
																<input type="hidden" name="userIdResume"
																	value="${ user.getId() }" />
																<label style="width : 100%;" class="file">
																	<input type="file" name="paticipantResume"
																		required />
																	<span class="file-custom"></span>
																</label>
																<button class="btn btn-primary btn-sm col-12 mt-2"
																	type="submit">Send Resume</button>
															</form>
														</div>
													...
													<c:otherwise>
														...
														<a class="btn btn-primary btn-sm col-12 mb-3" id="viewResume"
															href="${pageContext.request.contextPath }/assets/resumes/${sentResume.getPath()}"
															target="_blank">View Resume</a>
														<form id="changeResumeForm" method="#" action="post">
															<input type="hidden" name="idResumeToChange"
																value="${ sentResume.getId() }" />
															<input type="hidden" name="pathResumeToChange"
																value="${ sentResume.getPath() }" />
															<label style="width : 100%;" class="file">
																<input type="file" name="paticipantResume" required />
																<span class="file-custom"></span>
															</label>
															<button class="btn btn-primary btn-sm col-12"
																type="submit">Change Resume</button>
														</form>
														<small id="resumeMessage" class="text-danger"></small>
													</c:otherwise>
												</c:choose>
													...
													<c:choose>
														<c:when test="${ standResume != null  }">
															<c:forEach items="${ standResume }" var="resume">
																<div
																	class="p-2 mb-0 small lh-sm border-bottom d-flex justify-content-between align-items-center">
																	<div>
																		<strong
																			class="d-block text-gray-dark">Name</strong><a
																			href="${pageContext.request.contextPath }/participant?id_p=${resume.getP_id()}"><span>${
																				resume.getP_name() }</span></a>
																	</div>
																	<a class="btn btn-primary btn-sm"
																		href="${pageContext.request.contextPath }/assets/resumes/${ resume.getPath() }"
					...
					<c:if test="${ standRep.isApproved() == true}">
						...
						<div class="row" style="margin-top: -15px;" id="mainMessagesSection">
							<c:forEach items="${ standRep.getEntreprise().getMessages() }" var="mainMessage">
								<c:if
									test="${ mainMessage.getPrivacy() == 0 || user.getId() == standRep.getId() || mainMessage.getWriter_id() == sessionScope.Connected.getId() }">
									<div class="col-xl-12 col-lg-12 col-md-12 mb-4">
										<div class="card border-left-primary border-0 shadow h-100 ">
													...
													<div class="col mr-2 justify-content-center">
														...
														<div style="float: left;">
															...
															<p class="card-text"><small class="text-muted">Sent by
																	<c:choose>
																		<c:when
																			test="${ mainMessage.getWriterStatus() == 3}">
																			<a
																				href="${pageContext.request.contextPath }/participant?id_p=${mainMessage.getWriter_id()}"><strong>${
																					mainMessage.getWriter_name() }
																				</strong></a>
																		</c:when>
																		<c:otherwise>
																			<strong>${ mainMessage.getWriter_name() }
																			</strong>
																		</c:otherwise>
																	</c:choose>
																	at <i>${ mainMessage.getMessage_date() }</i>
																</small>
																<c:if test="${ mainMessage.getPrivacy() == 1 }">
																	<span
																		class="badge bg-danger text-white">Private</span>
																</c:if>
																<span class="badge bg-success text-white"><span
																		id="reponsesNumb${ mainMessage.getId_message() }">
																		<c:choose>
																			<c:when
																				test="${ mainMessage.getResponses().size() > 0 }">
																				${mainMessage.getResponses().size()}
																			</c:when>
														...
														<button
															style="float: right; border-radius: 30px; width: 30px; height: 30px;"
															class="btn btn-primary btn-sm shadow" type="button"
															data-bs-toggle="collapse"
															id="collapse${ mainMessage.getId_message() }"
															data-bs-target="#main${ mainMessage.getId_message() }"
															aria-expanded="false" aria-controls="collapseExample">
															<i class="fa fa-angle-down"></i>
														</button> <br>
														</p>
														<div class="collapse collapse1"
															id="main${ mainMessage.getId_message() }">
															<c:forEach items="${ mainMessage.getResponses() }"
																var="response">
																<div class="card card-body">
																	${ response.getMessage() }
																	<p class="card-text"><small class="text-muted"> Sent
																			by
																			<c:choose>
																				<c:when
																					test="${ response.getWriterStatus() == 3}">
																					<a
																						href="${pageContext.request.contextPath }/participant?id_p=${response.getWriter_id()}"><strong>${
																							response.getWriter_name() }
																						</strong></a>
																				</c:when>
																				<c:otherwise>
																					<strong>${ response.getWriter_name()
																						} </strong>
																				</c:otherwise>
																			</c:choose>
																			at <i>${ response.getMessage_date() }</i>
																		</small></p>
																</div> <br>
															</c:forEach>
														</div>
											...
											<div class="card-footer">
												<form action="#" method="post" class="responseFrom"
													style="margin-bottom: -15px;">
													<div class="input-group mb-3">
														<input type="hidden" value="${standRep.getEntreprise().getId()}"
															name="stand">
														<input type="hidden" value="${user.getId() }" name="writer">
														<input type="hidden"
															value="${user.getName()} ${ user.getFname()}"
															name="writer_name">
														<input type="hidden" value="${mainMessage.getId_message()}"
															name="responseTo">
														<input type="hidden" value="${mainMessage.getPrivacy()}"
															name="privacy">
														...
														<button style="border-radius: 0px 5px 5px 0px;"
															class="btn btn-primary btn-sm" name="newMessage"
								...
								<div class="col-lg-12">
									<div style="float : right;">
										<div class="form-group">
											<select class="form-control bg-white shadow rounded-sm border-0"
												name="privacy"
												style="background-color: #f8f9fa; border: solid 1px #6c757d;"
												class="form-control-sm" id="sel1">
							...
							<div class="row">
								<div class="col-lg-12">
									<div class="form-group">
										...
										<input type="hidden" value="${user.getId() }" name="writer">
										<input type="hidden" value="${user.getName()} ${ user.getFname()}"
											name="writer_name">
										<textarea class="form-control shadow rounded-sm border-0" name="message"
											rows="3" required data-msg="Please write something for us"
											placeholder="Ask your question here..."></textarea>
										<div class="validate"></div>
										<span id="chatErrorMsg"></span>
									</div>
									<div style="float: left;" class="text-center"><input
											class="btn btn-primary btn-sm shadow" name="newMessage" type="submit"
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<form action="#" method="post" id="logoForm">
							<label style="width : 100%;" class="file">
								<input type="file" id="logoInp" name="logoImage" required
									aria-label="File browser example">
								<input type="hidden" name="logoInput">
								<span class="file-custom"></span>
							</label>
							<button type="submit" class="btn btn-sm btn-primary shadow-sm" name="submitLogo">Change
								Logo</button>
						</form> <br>
						<form action="#" method="post" id="bannerForm">
							<label style="width : 100%;" class="file">
								<input type="file" id="bannerInp" name="bannerImage" required
									aria-label="File browser example">
		...
		<c:if test="${ sessionScope.firstLog == true}">
			<script>
				swal("Welcome!", "Your application has been submitted, You'll recieve the result once an admin treats it.", "success");
			</script>
							...
							default:
								...
								setTimeout(function () {
									$("#resumeFormDiv").html("<a class='btn btn-primary btn-sm' href='${pageContext.request.contextPath}/assets/resumes/"
										+ data + "' target='_blank'>View Resume</a>");
								}, 2000);
						}
					}
				});
