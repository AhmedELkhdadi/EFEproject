...
public class ParticipantDaoImpl implements ParticipantDao {
	...
	public boolean addParticipant(Participant participant) {
		...
		try {
			...
			prepStatement = conn.prepareStatement(
					"INSERT INTO participants(id_p,login,password,name,fname,email,phone,level,institution,studyField,sexe,age,imagePath) Values(?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			prepStatement.setString(9, participant.getInstitution()); // so we don't have to get them with a query later
																		// to show them
	...
	public boolean authenticate(Participant participant) {
		...
		try {
			...
			ResultSet result = query
					.executeQuery("SELECT * FROM participants WHERE login='" + log + "' AND password='" + pw + "';");
	...
	public Participant selectPart(String id) {
		...
		try {
			...
			if (result.next() == false)
				...
				participant = new Participant(id, result.getString("name"), result.getString("fname"),
						result.getString("email"), result.getString("phone"), result.getString("institution"),
						result.getString("level"), result.getString("studyField"), result.getString("sexe"),
						result.getInt("age"), result.getString("imagePath"), 3, result.getString("training"),
						result.getString("awards"), result.getString("techSkills"), result.getString("softSkills"),
						result.getString("langages"), result.getString("description"), result.getString("intrests"),
	...
	public List<Participant> SelectAll() {
		...
		try {
			...
			while (res.next()) {
				Participant part = new Participant(res.getString("id_p"), res.getString("name"), res.getString("fname"),
						res.getString("email"), res.getString("phone"), res.getString("institution"),
						res.getString("level"), res.getString("studyField"), res.getString("sexe"), res.getInt("age"),
						res.getString("imagePath"), 3, res.getString("facebook"), res.getString("gitHub"),
	...
	public Map<String, String> getStatistics() {
		...
		try {
			...
			query = conn.createStatement();
			ResultSet res = query
					.executeQuery("SELECT sexe,COUNT(*) as count FROM participants GROUP BY sexe ORDER BY count DESC;");
			...
			res = query.executeQuery(
					"SELECT level,COUNT(*) as count FROM participants GROUP BY level ORDER BY count DESC;");
			...
			res = query.executeQuery(
					"SELECT institution,COUNT(*) as count FROM participants GROUP BY institution ORDER BY count DESC;");
	...
	public boolean modifyInfo(String id, String training, String awards, String techSkills, String softSkills,
			String langages, String description, String intrests, String email, String phone, String facebook,
		...
		try {
			conn = daoFactory.getConnection();
			prepStatement = conn.prepareStatement(
					"UPDATE participants SET training=?, awards=?, techSkills=?, softskills=?, langages=?, description=? "
