...
public class RepresentativeDaoImpl implements RepresentativeDao{
	...
	public boolean addRep(Representative rep) {
		...
		try {
			...
			prepStmt = conn.prepareStatement("INSERT INTO representatives(id_r,login,password,name,fname,email,phone) Values(?,?,?,?,?,?,?)");
			UUID uid = UUID.randomUUID();
			String id = "" + uid; // this is for casting, since direct casting is not possible, and there is no setUUID in jdbc
			rep.setId(id);
	...
	public boolean authentify(Representative rep) {
		Connection conn = null;
		...
		try {
			...
			ResultSet result = query.executeQuery("SELECT * FROM representatives WHERE login='"+log+"' AND password='"+pw+"';");
	...
	public Representative selectRep(String id_r) {
		...
		try { 
			conn = daoFactory.getConnection();
			query = conn.createStatement();
			ResultSet repRes = query.executeQuery("SELECT id_r,name,fname,email,phone,approved FROM representatives WHERE id_r='"+id_r+"';");
			if(!repRes.next()) {
				return null;
			}
			else
			{
				...
				if(ent == null)
					...
					{
						rep = new Representative(id_r,repRes.getString("name"),repRes.getString("fname") , repRes.getString("email"), repRes.getString("phone"),ent,2);
						boolean approved = repRes.getBoolean("approved");
						if(!repRes.wasNull())	// if approved is not null in database we set approved in rep, else we leave it null to mean that it is still on wait 
