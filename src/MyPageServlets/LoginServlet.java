	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (ajax) {
			...
			if (verified) {
				...
				rep.setLogin(log);
				rep.setPassword(pw);
				verified = representativeDao.authenticate(rep); // Entreprise is set for representative in the function
																// authentify()
				if (verified) {
					HttpSession Sess = request.getSession();
		...
		/*
		 * if(!connected) this.doGet(request, response); else {
		 * response.sendRedirect(request.getContextPath()+ "/"); // we use redirect
		 * because forward() will keep the post data even after going to index }
		 */
